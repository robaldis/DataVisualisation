function ClimateChange() {
  ...
  this.draw = function() {
    ...
    if (mouseX > this.layout.leftMargin && mouseX < this.layout.rightMargin) {
      var year = round(this.mapWidthToYear(mouseX));
      var temp = this.data.findRow(year.toString(), 'year').obj.temperature
      fill(0)
      strokeWeight(1)
      stroke(1)
      line(mouseX,this.layout.topMargin,  mouseX,this.layout.bottomMargin)
      text(`Temperature:\t${temp}°C`, mouseX - 100, mouseY);
      text(`Year: \t\t${year}`, mouseX-100,mouseY + 20)
    }
  ...
  this.mapWidthToYear = function (value) {
    return map(
                value,
